#!/usr/bin/env python3
"""
SHA-256 SAT Solver v3 — Benchmark with process-level timeout.
"""
...
def _solve_worker(q, target_hash_hex, msg_len, charset, n_rounds, use_backward, use_stvorochne):
    """Worker process for solving with clean kill support."""
    ...
    solver.delete()
    q.put((result_msg, solve_time, s.nv, len(s.cl), n_pinned, n_stv, build_time))


def solve_with_timeout(target_hash_hex, msg_len, charset='ascii', n_rounds=64,
                       use_backward=True, use_stvorochne=True, timeout_sec=120):
    """Solve with hard process timeout."""
    q = Queue()
    p = Process(target=_solve_worker,
                args=(q, target_hash_hex, msg_len, charset, n_rounds, use_backward, use_stvorochne))
    p.start()
    ...
    if p.is_alive():
        ...
        if p.is_alive():
            p.kill()
            p.join()
        return None, timeout_sec, 0, 0, 0, 0, 0, True  # timed out

    if not q.empty():
        result_msg, solve_time, nv, nc, np, ns, bt = q.get()
        return result_msg, solve_time, nv, nc, np, ns, bt, False
...
# ================================================================
print("╔════════════════════════════════════════════════════════════╗")
print("║  SHA-256 SOLVER v3 — BENCHMARK (with hard timeouts)       ║")
print("╚════════════════════════════════════════════════════════════╝")

TIMEOUT = 150  # 2.5 min per test

# ========== TEST 1: v2 vs v3 on "Hi" (64R) ==========
print("\n=== TEST 1: v2 vs v3 on 'Hi' (2B=16bits, 64R) ===")
target = hashlib.sha256(b"Hi").hexdigest()

t_base = None
for name, bk, stv in [("v2 baseline", False, False),
                       ("v3 bkwd", True, False),
                       ("v3 bkwd+stv", True, True)]:
    r, t, nv, nc, np, ns, bt, to = solve_with_timeout(target, 2, 'ascii', 64, bk, stv, TIMEOUT)
    if to:
        print(f"  {name:25s}: TIMEOUT >{TIMEOUT}s")
    else:
        if t_base is None: t_base = t
        sp = f"({t_base/t:.2f}×)" if t > 0 else ""
        ok = ""
        if r:
            check = hashlib.sha256(r).hexdigest() == target
            try: ok = f"'{r.decode()}' {'ok' if check else 'FAIL'}"
            except: ok = r.hex()
        print(f"  {name:25s}: {ok:15s} {t:6.1f}s {nv}v/{nc}c pin={np} stv={ns} {sp}")

# ========== TEST 2: Round scaling for Hi (2B) ==========
print("\n=== TEST 2: Round scaling — 'Hi' (2B=16bits) ===")
print(f"  {'NR':>3s}  {'v2':>8s} {'v3':>8s} {'ratio':>6s}")
print(f"  {'─'*3}  {'─'*8} {'─'*8} {'─'*6}")

for NR in [8, 12, 16, 20, 24, 28, 32, 40, 48, 56, 64]:
    target = get_reduced_hash(b"Hi", NR) if NR < 64 else hashlib.sha256(b"Hi").hexdigest()
    bk = (NR == 64)

    r2, t2, *_, to2 = solve_with_timeout(target, 2, 'ascii', NR, False, False, TIMEOUT)
    r3, t3, *_, to3 = solve_with_timeout(target, 2, 'ascii', NR, bk, True, TIMEOUT)

    t2s = f">{TIMEOUT}" if to2 else f"{t2:.2f}"
    t3s = f">{TIMEOUT}" if to3 else f"{t3:.2f}"
    sp = ""
    if not to2 and not to3 and t3 > 0.001:
        sp = f"{t2/t3:.2f}×"
    print(f"  {NR:3d}  {t2s:>8s} {t3s:>8s} {sp:>6s}")

    if to2 and to3:
        break

# ========== TEST 3: Message size scaling at fixed rounds ==========
print("\n=== TEST 3: Message size scaling at 24R (v3) ===")
print(f"  {'Msg':6s} {'bits':>4s} {'v2':>8s} {'v3':>8s} {'ratio':>6s}")
print(f"  {'─'*6} {'─'*4} {'─'*8} {'─'*8} {'─'*6}")

for msg in [b"A", b"Hi", b"Abc", b"Test", b"Hello"]:
    NR = 24
    target = get_reduced_hash(msg, NR)
    bits = len(msg) * 8

    r2, t2, *_, to2 = solve_with_timeout(target, len(msg), 'ascii', NR, False, False, TIMEOUT)
    r3, t3, *_, to3 = solve_with_timeout(target, len(msg), 'ascii', NR, False, True, TIMEOUT)
    ...
    r_any = r3 or r2
    if r_any:
        try: res = f"'{r_any.decode()}'"
        except: res = r_any.hex()[:12]
    print(f"  {msg.decode():6s} {bits:4d} {t2s:>8s} {t3s:>8s} {sp:>6s} {res}")
    if to2 and to3:
        break

# ========== TEST 4: 1-byte brute force comparison ==========
print("\n=== TEST 4: 1-byte 'A' (8 bits) full rounds ===")
target = hashlib.sha256(b"A").hexdigest()

r2, t2, *_, to2 = solve_with_timeout(target, 1, 'ascii', 64, False, False, TIMEOUT)
r3, t3, *_, to3 = solve_with_timeout(target, 1, 'ascii', 64, True, True, TIMEOUT)
...
if r3:
    ...
    try: print(f"  Found: '{r3.decode()}' hash={'ok' if check else 'FAIL'}")
    except: print(f"  Found: {r3.hex()}")
