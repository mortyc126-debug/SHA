#!/usr/bin/env python3
"""
SHA-256 SAT Solver v3 — Enhanced with NK instruments.

v2: basic SAT encoding, 64 rounds, 98s for 2-byte "Hi".
v3 additions:
  1. Backward chain: pin a[57..64] from target H (384 free bits)
  2. Створочне: e[r] = f(a-chain) redundant constraints
  3. A[1] linearity: A[1] = 0xfc08884d + W[0]
  4. Schedule inversion: W[0..2] from W[3..18]
  5. Test on longer messages (3, 4, 5 bytes)
"""
...
class SAT:
    ...
    def waddm(self,ws):
        ...
        return r
    def wsub(self,a,b):
        """a - b mod 2^32 via a = result + b → result = a - b."""
        # Encode: result + b = a (mod 2^32)
        result = self.word()
        sum_word = self.wadd(result, b)
        self.weq(sum_word, a)
        return result
...
def backward_chain_from_H(target_H):
    """Compute a[57..64] and e[61..64] from H. Returns dict of known values."""
    ak = {}; ek = {}
    for i in range(4):
...
def solve_sha256_v3(target_hash_hex, msg_len, charset='ascii', n_rounds=64,
                    use_backward=True, use_stvorochne=True, verbose=True):
    target_H = [int(target_hash_hex[i*8:(i+1)*8], 16) for i in range(8)]

    if verbose:
        flags = []
        if use_backward: flags.append("backward")
        if use_stvorochne: flags.append("створочне")
        print(f"Цель: {target_hash_hex[:16]}..., {msg_len}B, {charset}, {n_rounds}R, +{'+'.join(flags)}")
    ...
    # NK INSTRUMENT 1: Pin backward-chain values
    n_pinned = 0
    if use_backward and n_rounds == 64:
    ...
    # NK INSTRUMENT 2: Створочне redundant constraints
    n_stvorochne = 0
    if use_stvorochne:
        for r in range(4, min(n_rounds+1, 12)):
            if r in a_vars and r-1 in a_vars and r-2 in a_vars and r-3 in a_vars and r-4 in a_vars and r in e_vars:
    ...
    if verbose:
        print(f"  SAT: {s.nv} vars, {len(s.cl)} clauses, {n_pinned} pinned, {n_stvorochne} створочне, build={build_time:.2f}s")
    ...
    if ok:
        ...
        if verbose:
            check = hashlib.sha256(result_msg).hexdigest() if n_rounds==64 else None
            try: msg_str = f"'{result_msg.decode()}'"
            except: msg_str = result_msg.hex()
            verified = f" hash={'✓' if check==target_hash_hex else '✗'}" if check else ""
            print(f"  FOUND: {msg_str} in {solve_time:.2f}s{verified}")
    else:
        if verbose:
            print(f"  UNSAT in {solve_time:.2f}s")
...
print("╔════════════════════════════════════════════════════════════╗")
print("║  SHA-256 HYBRID SOLVER v3 + NK INSTRUMENTS                ║")
print("╚════════════════════════════════════════════════════════════╝")
...
# Test 1: Compare v2 vs v3 on "Hi" (64 rounds)
print("\n=== TEST 1: v2 vs v3 on 'Hi' (64R) ===")

target = hashlib.sha256(b"Hi").hexdigest()

print("\nv2 (baseline):")
_, t_v2 = solve_sha256_v3(target, 2, 'ascii', 64, use_backward=False, use_stvorochne=False)

print("\nv3 (backward only):")
_, t_v3b = solve_sha256_v3(target, 2, 'ascii', 64, use_backward=True, use_stvorochne=False)

print("\nv3 (backward + створочне):")
_, t_v3bs = solve_sha256_v3(target, 2, 'ascii', 64, use_backward=True, use_stvorochne=True)

if t_v2 > 0:
    print(f"\n  Speedup backward: {t_v2/t_v3b:.2f}×")
    print(f"  Speedup backward+створочне: {t_v2/t_v3bs:.2f}×")


# Test 2: Longer messages
print("\n=== TEST 2: Longer messages (64R, v3 full) ===")

for msg in [b"Hi", b"Abc", b"Test", b"Hello"]:
    target = hashlib.sha256(msg).hexdigest()
    print(f"\nMessage '{msg.decode()}' ({len(msg)} bytes, {len(msg)*8} bits freedom):")
    result, t = solve_sha256_v3(target, len(msg), 'ascii', 64,
                                 use_backward=True, use_stvorochne=True)
    if result:
        matched = "✓ EXACT" if result == msg else f"(alt preimage: {result})"
        print(f"  {matched}")
    if t > 120:
        print("  Stopping (>2min)")
        break
...
# Test 3: Reduced rounds with longer messages
print("\n=== TEST 3: Reduced rounds, 5-byte message ===")

for NR in [8, 16, 24, 32, 64]:
    msg = b"Hello"
    target = get_reduced_hash(msg, NR) if NR < 64 else hashlib.sha256(msg).hexdigest()
    print(f"\n{NR}R, 'Hello' (5B = 40 bits):")
    result, t = solve_sha256_v3(target, len(msg), 'ascii', NR,
                                 use_backward=(NR==64), use_stvorochne=True)
    if result:
        matched = "✓" if result == msg else f"alt: {result}"
        print(f"  {matched}")
    if t > 120:
        print("  Stopping")
        break
